            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                >
                    ...
                    <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                        ...
                        <div className="flex flex-col flex-1 p-4 border-b md:border-b-0 md:border-r border-slate-200 overflow-hidden">
                            ...
                            <div className="relative mb-3">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-colors"
                                />
                            </div>
                            {loading ? (
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-xs text-slate-500">R$ {Number(item.preco_sugerido).toFixed(2)}</p>
                                                    </div>
                                                    <button
                        ...
                        <div className="flex flex-col flex-1 p-4 overflow-hidden">
                            <h3 className="text-sm font-semibold text-slate-700 mb-3">
                                Itens neste Grupo
                                <span className="ml-2 text-xs font-normal text-slate-500">({vinculados.length})</span>
                            </h3>
                            <div className="overflow-y-auto flex-1 space-y-1.5 pr-1">
                                {vinculados.length === 0 ? (
                                    <div className="flex-1 flex flex-col items-center justify-center py-10 text-slate-500 text-sm text-center">
                                        <ChefHat className="w-10 h-10 mb-3 text-slate-300" />
                                        <p>Nenhum item neste grupo ainda.</p>
                                        <p className="text-xs mt-1 text-slate-400">Adicione sabores da biblioteca ao lado.</p>
                                    </div>
                                ) : (
                    ...
                    <div className="p-4 border-t border-slate-200 flex-shrink-0">
                        <button
                            onClick={onClose}
                            className="w-full py-2.5 rounded-xl bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 hover:border-slate-300 transition-colors text-sm font-medium"
                        >
                            Fechar
                        </button>
                    </div>
