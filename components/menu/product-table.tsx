...
export default function ProductTable({
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden overflow-x-auto custom-scrollbar relative min-h-[300px]">
            {loading && (
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-200">
                        ...
                        <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Disponibilidade</th>
                ...
                <tbody className="divide-y divide-slate-100">
                    {paginatedProducts.length > 0 ? paginatedProducts.map((product) => {
                        ...
                        return (
                            <tr key={product.id} className={cn("hover:bg-slate-50/50 transition-colors group", isEffectivelyUnavailable && "opacity-75")}>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-4">
                                        <div className={cn(
                                            "size-12 rounded-lg bg-slate-100 overflow-hidden border border-slate-200 shrink-0 relative",
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-sm font-semibold text-slate-900 border-b-0">{product.nome}</span>
                                            <span className="text-xs text-slate-500">Cod: {product.id}</span>
                                            {user?.controle_estoque && maxMakers !== -1 && (
                                                <span className={cn("text-[10px] sm:text-xs font-bold mt-1 max-w-fit px-1.5 py-0.5 rounded-md flex items-center gap-1", isOutOfStock ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600")}>
                                ...
                                <td className="px-6 py-4">
                                    <span className="text-sm font-bold text-slate-900">R$ {product.preco}</span>
                                </td>
                                <td className="px-6 py-4">
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        ...
                                        />
                                        <div className={cn("w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all", product.disponivel ? "peer-checked:bg-primary" : "", isOutOfStock && "opacity-50 cursor-not-allowed")}></div>
                                    </label>
                                </td>
                        ...
                        <tr>
                            <td colSpan={5} className="px-6 py-12 text-center text-slate-500">Nenhum produto encontrado.</td>
                        </tr>
                    )}
                </tbody>
            </table>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
                <div className="px-4 sm:px-6 py-4 bg-slate-50/50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <span className="text-[10px] sm:text-xs text-slate-500 font-medium tracking-wide uppercase text-center sm:text-left">
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        currentPage === page ? "bg-primary text-white" : "hover:bg-slate-200 text-slate-600"
                                    )}
                                >
                                    {page}
                                </button>
