 ...
 * GET /api/cron/remarketing/processar?key=SUA_CHAVE (para testes no navegador)
 * 
...
async function handleRequest(cronKey: string | null) {
  ...
  const diasSemana = Array.isArray(config.dias_semana) ? config.dias_semana : JSON.parse(config.dias_semana as unknown as string || '[]');
  if (!diasSemana.includes(diaAtual)) {
    return NextResponse.json({
      success: true,
    ...
    try {
      ...
      if (response.ok) {
        ...
        await pg.raw(`
          ...
          SET ultima_msg_remarketing = NOW(), 
              total_msgs_enviadas = COALESCE(total_msgs_enviadas, 0) + 1 
          WHERE id = $1
        `, [item.contato_id]);
