 ...
 * GET /api/cron/remarketing/agendar?key=SUA_CHAVE (para testes no navegador)
 * 
...
async function handleRequest(cronKey: string | null) {
  ...
  const debugInfo: { categoria: string; contatos: number; mensagem: string | null }[] = [];
  ...
  for (const categoria of categorias) {
    ...
    let mensagemUsada: string | null = null;
  ...
  await pg.create(REMARKETING_HISTORICO_TABLE, {
    ...
    dados: JSON.stringify({ totalAgendados, debugInfo }),
    created_at: new Date().toISOString(),
  });
